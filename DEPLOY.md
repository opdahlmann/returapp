...
| --- | --- | --- | --- | --- | --- |
| `returapp-dev-api` | `opd` | `infra/api/Dockerfile` | `.` | 8080 | – (ingen offentlig) |
| `returapp-dev-web` | `opd` | `infra/web/Dockerfile` | `.` | 80 | `dev.<domene>` |
| `returapp-api` | `main` | `infra/api/Dockerfile` | `.` | 8080 | – (ingen offentlig) |
| `returapp-web` | `main` | `infra/web/Dockerfile` | `.` | 80 | `<domene>` |
...
- Nettleseren snakker bare med `web`. `web` proxyer `/api/*` til API-appen.
...
App__BaseUrl=<https://dev.<domene> | https://<domene>>
...
1. **DNS**: `dev.<domene>` og `<domene>` peker på Dokploy-vertens IP **før** første deploy (ellers feiler Let's Encrypt).
2. **Prosjekt** `returapp` i Dokploy.
3. **API først**: Create Application → innstillinger fra § 1 → Environment fra § 3 → Deploy. Noter tjenestenavnet.
...
6. **Helsesjekk og rollback**: imagene har `HEALTHCHECK` (API: `/api/ready`). Sett Swarm update config til rollback når en ny versjon ikke blir frisk.
7. Gjenta 3–6 for det andre miljøet.

## 5 · Røyktest etter deploy

1. `https://<domene>/api/health` → 200 (API-et lever, proxyen virker).
2. `https://<domene>/api/ready` → 200 (databasen svarer).
3. Åpne en dyp lenke, f.eks. `https://<domene>/p/R-2041` → appen laster (SPA-fallback).
4. Logg inn. Dev: brukerne øverst i `README.md`.
...
| Web laster, men `/api` gir 502 | `API_UPSTREAM` peker på feil eller gammelt tjenestenavn | Les tjenestenavnet i API-appens Logs-fane, Redeploy web |
...
| `/api/ready` 503, innlogging gir 500 | `authMechanism=DEFAULT` i tilkoblingsstrengen (Compass legger den på) | Fjern parameteren |

...
| Bilder og filer | Dokumenter i collection `files` (`data` = BinData, maks 16 MB per dokument) | Krav: ingen lokal disk (se prinsippet over). Enklere enn GridFS: ett dokument per fil, vanlig `Find`/`InsertOne`. Maks opplasting 10 MB. Ved opplasting (SkiaSharp, MIT – ImageSharp 4 krever kommersiell lisensnøkkel ved bygg): originalen roteres etter EXIF og skaleres ned til maks 2048 px på lengste side (JPEG, kvalitet 85, EXIF inkl. GPS fjernes), thumbnail 400 px lagres som eget dokument. Et mobilbilde havner da på ~0,3–0,8 MB i databasen. |
...
- `public.ecr.aws/docker/library/node:24-alpine` (ECR Public, ingen Docker Hub-ratelimit) → `npm ci` mot innsjekket lockfile → `ng build --configuration ${NG_CONFIGURATION}` → `public.ecr.aws/nginx/nginx-unprivileged:alpine` (ikke-root, fase 13).
...
1. `dotnet new sln` (gir `Returapp.slnx` i .NET 10), `global.json` med SDK `10.0.102`, `dotnet new web -n Returapp.Api` (Minimal API), `dotnet new xunit -n Returapp.Api.Tests`. Pakker legges til i fasen de brukes: `MongoDB.Driver` (fase 1), `Microsoft.AspNetCore.Authentication.JwtBearer` (fase 2), `SkiaSharp` + `SkiaSharp.NativeAssets.Linux.NoDependencies`, `QuestPDF`, `QRCoder` (fase 5). `PasswordHasher<T>` ligger allerede i ASP.NET Core-rammeverket, så `Microsoft.Extensions.Identity.Core` trengs ikke som pakke. Test: `Microsoft.AspNetCore.Mvc.Testing`.
...
4. `Services/Weight.cs`: `EstimateKg(category, unit, qty)` = `kgPerUnit[unit] * qty`, `Co2(kg)` = `kg * 0.9` (fast faktor, samme i frontend).
...
| 15 | PDF-lisens | QuestPDF brukes under Community-lisensen (gratis for virksomheter under 1 MUSD i årlig omsetning). Overstiger Returapp det, må lisens kjøpes eller `Services/Pdf.cs` skrives om til f.eks. PDFsharp/MigraDoc (MIT). Bildebehandling bruker SkiaSharp (MIT) fordi ImageSharp 4 krever betalt lisensnøkkel. |
...
| 2026-09-15 | Fase 5 funn: SkiaSharp i stedet for ImageSharp (lisens), bilder hentes i frontend med token som blob (`ra-photo`), bildestripen på detalj får `flex-shrink:0` (i prototypen kollapser den til 0 px – bevisst avvik, stripen skal vises), tidslinjen leser hvor langt ordren kom fra `statusLog` (avvik etter planlagt viser planlagt som gjort), seed legger genererte bilder på demo-ordrene i prototypens antall, `POST /api/tips` laget her, `POST /api/dev/cleanup` (kun dev) rydder e2e-ordre merket `[e2e]` og SMS-testbrukere, `App__Geocode=false` i tester, `App__AuthRateLimitPerMinute=1000` i `.env.development` (e2e logger inn mange ganger), merkelapp/kvittering har bare tittel i header som prototypen. QuestPDF bruker `/tmp` til intern cache (tmpfs i read-only-container, ingen data). Verifisert: 37 backend, 19 Vitest, 12 e2e, opplasting og PDF gjennom nginx mot read-only API-container, visuell sammenligning av hjem, liste, wizard steg 1–5, detalj og merkelapp. | 2.2, fase 1, 5 (risiko 15) |
| 2026-09-15 | Fase 6 funn: tildeling fra børs er atomisk (filter `open && ny`, første sjåfør vinner, ellers 409); `GET /api/routes` uten dato velger første dag fra i dag med stopp; stopp uten koordinater geokodes når ruten vises (luftlinje × 1,3); statistikk beregnes i minnet over firmaets ordre siste 120 dager (`ponytail`-merket, $facet ved stort volum), kategori «Annet» slås sammen med restgruppen; avdelinger adresseres med indeks (`PUT/DELETE /departments/{index}`); invitasjon av sjåfør kan ta med kjøretøy/områder; badges på bunnmenyen lastes i shellet ved hver fanebytting; telefonformat 3-2-3 for mobil og 2-2-2-2 for fasttelefon. Verifisert: 44 backend-tester (tildeling, børs-kappløp, rute-rekkefølge, statistikk mot egne data), 14 e2e (bl.a. tildel Kari → ruteplan → flytt → send), visuell sammenligning av innboks, tildel-sheet, ruteplan, firma, sjåfører, avdelinger og statistikk. | fase 6 |
| 2026-09-15 | Fase 7 funn: «Dagens stopp» bruker `GET /api/routes?date=i dag` (samme rekkefølge som admin setter), «Gjort i dag» filtreres fra `scope=driver`; `complete` godtar tildelt/planlagt/underveis og knytter bildene som `pickedPhotos`; kvittering sendes som e-post med PDF-vedlegg (bygget i minnet) når giver har e-post på, ellers SMS med lenke; QR-skanning i sheet med `BarcodeDetector` og dynamisk importert `@zxing/browser` som reserve; Rute-skjermen viser «Neste rute: …» når ruten ikke er i dag; e2e kjører `ng serve --no-hmr` (HMR byttet ut komponenter midt i en test). Verifisert: 47 backend (start, bilde påkrevd, 20 av 24 → 600 kg, kvitterings-e-post med PDF, avviksregler, sjåfør ser/rører bare egne), 16 e2e (hel henting med bilde og kvittering, ta oppdrag fra børs, avvik synlig for admin), visuell sammenligning av Henting og Rute. | fase 7 |
| 2026-09-15 | Fase 8 funn: søknad lagrer kontaktperson (navn/e-post/telefon) på firmaet, og godkjenning uten eksisterende admin sender invitasjon som retur-admin til kontaktpersonen; godkjenning kjører samme dekningslogikk som fase 4 (ventende ordre får firma); systemvarsel lagres som in-app-varsel til mottakerne og vises som banner øverst på rotskjermer (lukkes per økt); brukersheet viser firmavelger bare når sjåfør/admin mangler firma; hentefirma listes i registreringsrekkefølge som i designet; superbrukers firmadetalj er egen side `/s/companies/:id`; dev-cleanup rydder også e2e-firma («E2E …»), `[e2e]`-varsler og tilhørende supportsaker. Verifisert: 54 backend-tester (godkjenning med dekning og invitasjon, avvisning, firmabytte, roller og egen superbruker-rolle, deaktivering, varsel til hentefirma, support, søknad), 18 e2e (godkjenn → postnr-oversikt → ordre får firma; roller; banner hos giver), visuell sammenligning av oversikt, ordre, firma-sheet, hentefirma, brukere, bruker-sheet og support. | fase 8 |
| 2026-09-15 | Fase 9 funn: meldinger varsler motpart (giver → tildelt sjåfør, ellers firmaets admins; sjåfør/admin → giver) med in-app + push, og SMS bare for første melding per tråd og mottaker per 10 min (gjest får SMS med samme sperre via `guestSmsAt`); `Push__Provider` (Console|WebPush) velger sender, og push-payload bruker Angular service workers `notification`-format med `onActionClick` → `navigateLastFocusedOrOpen` til `/p/{id}` eller `/notifications`, så ingen egen SW-kode trengs; abonnement krever https-endepunkt, maks 5 per bruker, fjernes ved 404/410; varselsiden markerer alle som lest ved åpning og meldingsvarsler åpner tråden; bjelleprikk hentes sammen med tab-badges; push-bryteren i profil viser toast når service worker ikke er aktiv (dev-server). Polling 5 s mens fanen er synlig. Verifisert: 57 backend-tester (melding når sjåfør med push og SMS-sperre, ikke annen giver; varsler lest/ulest; ugyldig abonnement avvist), 19 e2e (giver skriver → sjåfør ser og svarer i egen kontekst → giver ser svaret, badge på Meldinger, varsel åpner tråden, prikk forsvinner), visuell sammenligning av meldingsliste og tråd. | fase 9 |
| 2026-09-15 | Fase 10 funn: eksport er ett endepunkt `GET /api/export/pickups.{csv|xlsx|pdf}` (scope `mine` for giver/gjest, `company` bare for admin i eget firma; `from`/`to` filtrerer på opprettet dato i norsk tid); tekst fra brukere som starter som formel (`= + - @`) får `'` foran i CSV, og settes alltid som tekst (aldri formel) i Excel; ClosedXML (MIT) med faste kolonnebredder fordi `AdjustToContents` krever fonter i containeren; miljøeffekt per kategori (ark «Per kategori», PDF-forside) regnes bare for hentede ordre, samme som statistikken; PDF-rapporten gjenbruker kvitteringssiden fra fase 6; «Del» på kvittering og merkelapp deler PDF-en som fil når `navigator.canShare({files})` støtter det, ellers lenken. Verifisert: 61 backend-tester (BOM/semikolon/rader/formelvern/periode, to Excel-ark, PDF i minnet, admin ser bare eget firma, giver får 403 på firma), 21 e2e (giver CSV + Excel fra Historikk, admin PDF-rapport fra Statistikk), eksport i `--read-only`-container. | fase 10 |
| 2026-09-15 | Fase 11 funn: service worker cacher `/api/postnr/**` og `/api/photos/**` med performance, men kategorier, hentinger, meg, varsler, firma og ruter med freshness (3 s) – kategorier som performance ville vist superbrukerens endringer først etter et døgn; utlogging sletter service workerens API-cacher så neste bruker på enheten ikke ser forrige brukers data offline; «Ny versjon tilgjengelig» med «Oppdater» vises som vedvarende toast; ark får fokus ved åpning (arket selv, ikke første felt, så tastaturet ikke spretter opp), Tab sykler i arket og fokus går tilbake ved lukking; nginx manglet gzip (Lighthouse ytelse 79 → 98); `errorText` ga allerede «Ingen nettverk – prøv igjen» ved status 0, så eget `online`-signal ble ikke nødvendig; `@defer` på skjematisk kart og `NgOptimizedImage` droppet (kartet er en liten SVG i samme lazy chunk, bildene hentes som blob med token), bilder får `decoding=async`; budsjett initial 400 kB varsel / 500 kB feil (nå 312 kB). Lighthouse 12 har ikke lenger PWA-kategori, så CI-kravet «PWA ≥ 90» utgår – målt i stedet ytelse 98 / tilgjengelighet 94 / beste praksis 100 på `/login` mot produksjonscontainer. Dokumenterte avvik: iOS får 16px i felt (designet har 15px) for å unngå auto-zoom; innloggingens «eller»-skille (#829d8a på #1b4d2b, 3,3:1) og `--mu` på `--sf2` i lys modus (4,3:1) ligger under 4,5:1 i designet og er ikke endret; mørk modus har ≥ 5,5:1 på alle tokenpar. Verifisert: 19 unit, 23 e2e mot nginx + `--read-only` API-container (offline: cachet liste etter reload uten nett, toast ved lagring, cache tømt ved utlogging; fokusfelle og Escape), 22 e2e mot dev-server (offline-testen hoppes over uten service worker). | fase 11 |
| 2026-09-15 | Fase 12 funn: referansebildene (46 skjermer × lys/mørk = 92) tas fra prototypen med `CAPTURE=1 npx playwright test --project reference`; sammenligningen er egen pixelmatch-port i nettleseren (canvas, YIQ 0.1, antialiasing telles ikke) i stedet for `toHaveScreenshot`, fordi masker må gjelde både referanse og app og referansen har telefonramme – ingen nye avhengigheter; terskelen er strammet fra 2 % til 1 % (mutasjonssjekk: 14 → 18 px gap gir 1,3 %). Prototypen har en layoutfeil: flex-kolonner med `overflow:auto` krymper barna når innholdet er høyere enn skjermen (søkefelt 24 px, filterchips og bildestripe 0 px) – appen fikser dette én gang (`.ra-scroll`, barn krymper aldri, erstatter enkeltfiksene fra fase 5/8 og rettet også brukerlistens søkefelt), og referansen tas med samme regel. Demo-data: nytt `POST /api/dev/reset-demo` (bare med dev-endepunkter) setter demo-ordre, -brukere, -firma, kategorier og support tilbake med datoer fra i dag og beholder bilder og økter, så visuelle tester ikke avhenger av når databasen ble seedet; R-2041 og R-2037 seedes på i dag (designet viser dem som Karis stopp «I dag» og på ruten); hentede demo-ordre får bildene som dokumentasjon ved henting; brukerlisten sorteres i registreringsrekkefølge som i designet. Funne avvik rettet: QR-boksen hadde fast høyde (designet: innholdshøyde), siste rad i brukerarket manglet skillelinje, avdelinger skriver ikke «mottak av». Nye e2e: invitasjon på e-post → aktiver → glemt passord → nytt passord, support (sak → svar → lukk → varsel), tildel firma fra «Uten firma». CI: `images`-jobben er erstattet av `e2e` (etter `backend`, så de ikke skriver til dev-databasen samtidig) som bygger begge images, starter API `--read-only` og web med `docker run` på felles nettverk og kjører Playwright i chromium og webkit; visuelle tester bare i chromium; offline-testen hoppes over i webkit (Playwright WebKit kan ikke navigere offline via service worker). WebKit avdekket at fokusfellen var avhengig av nettleserens Tab-rekkefølge (Safari hopper over knapper) – Tab/Shift+Tab styres nå helt av appen. Verifisert: 62 backend-tester, 19 unit, 97 e2e mot containerne (chromium 52 inkl. 46 visuelle + webkit 45, 1 hoppet over), 71 mot dev-server. Avvik: `e2e/visual/ALLOWED_DIFFS.md`. | fase 12 |
| 2026-09-15 | Fase 13 funn: selve Dokploy-oppsettet (DNS, apper, domener, hemmeligheter, backup-cron) gjøres av eier og er beskrevet i `DEPLOY.md` § 4, 8 og 9; kodesiden er ferdig. API: JSON-logg til stdout utenfor Development (med norske tegn uten escaping), DataProtection-advarsler dempet (appen bruker egne JWT-er), Kestrel `MaxRequestBodySize` 10 MB som nginx (verifisert 413), HSTS 1 år via `UseHsts` (bare https via `X-Forwarded-Proto`, ikke localhost), `X-Content-Type-Options: nosniff` på alle svar, bilder med `Content-Disposition: inline`, advarsel i loggen når `App__DevEndpoints` er på, `POST /api/client-errors` (anonym, 20/min per IP, logges som `Klientfeil`, lagres ikke). Frontend: `ClientErrorHandler` sender maks 5 feil per sidevisning med sti der invitasjons-/reset-token er fjernet (`safePath`, unit-testet); `APP_VERSION` flyttet til `core/http.ts` så profil-modulen ikke havner i hovedbundelen. web: `nginx-unprivileged` (uid 101) på port 80 – Docker tillater lave porter i containerens eget nettverk, så Dokploy-porten er uendret; HSTS i hver location (nginx arver ikke `add_header` inn i blokker med egne). Rotasjon av `Jwt__Secret` logger ikke ut brukere (refresh-tokens er SHA-256 uten hemmelighet) – dokumentert hvordan alle logges ut. Verifisert: 63 backend-tester, 20 unit, 97 e2e mot de herdede containerne (chromium + webkit), 71 mot dev-server. | fase 13 |
| 2026-09-15 | Opprydding etter ponytail-audit (funn 1–16 i `PONYTAIL-AUDIT.md`): CO₂-faktoren er fast 0,9 på begge sider (`App__Co2Factor` fjernet – frontend brukte aldri variabelen, så tallene kunne sprike); `Jwt__Issuer`/`AccessMinutes`/`RefreshDays` er konstanter; ubrukte policyer `giver`/`driver`/`admin` fjernet (rollesjekk skjer med `Caller.Has`); `Caller.IsGuest` avledes av `gid`; `POST /api/support` flyttet inn i `Super.cs` sammen med de andre support-rutene; `PushPayload` inlinet i `Notifier`; `StoredFile.Size`/`ContentType`, `ProcessedImage.ThumbW/H` og `Pickup.Seq` fjernet (gamle dokumenter beholder feltene, `IgnoreExtraElements`); svarfeltene `pushDevices` og `companyPhone` fjernet; `@angular/forms`, `prettier` og `shell/empty.ts` fjernet; `MONTHS` deles fra `format.ts`; `num()` bruker `toLocaleString('nb-NO')` (hardt mellomrom); e2e-hjelperne `devCode`/`mailLink` deler én `devMatch`. Verifisert: 63 backend-tester, 20 unit, 71 e2e mot dev-server, produksjonsbygg uten advarsler. | opprydding |
